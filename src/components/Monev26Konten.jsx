...
// ─── Komponen Utama ────────────────────────────────────────────────
export default function Monev26Konten() {
  ...
  useEffect(() => {
    if (potiks.length > 0) {
      setIsLoading(true);
      setTimeout(() => {
        buildChartData(potiks, sortBy, filterRegion);
        setIsLoading(false);
      }, 50);
    }
  ...
  // ── Build data chart ─────────────────────────────────────────────
  const buildChartData = (currentList, sort, region) => {
    let filtered = region === 'all' ? currentList : currentList.filter(p => p.region === region);
  ...
  const getFilename = (ext) => {
    ...
    return `PotikMonitor_Export_Monev26_${date}_${time}.${ext}`;
  };
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Data Monev 26 Konten');

      // Sheet ringkasan
      const summarySheet = XLSX.utils.aoa_to_sheet([
        ['Laporan Aktivitas Pojok Statistik BPS Jawa Timur (Monev \'26)'],
        [],
        ['Kategori', 'Jumlah Konten', 'Persentase'],
        ['Infografis', summary.infografis, summary.total > 0 ? `${((summary.infografis / summary.total) * 100).toFixed(1)}%` : '0%'],
        ['Video',      summary.video,      summary.total > 0 ? `${((summary.video      / summary.total) * 100).toFixed(1)}%` : '0%'],
        ['Edukasi',    summary.edukasi,    summary.total > 0 ? `${((summary.edukasi    / summary.total) * 100).toFixed(1)}%` : '0%'],
        ['Kegiatan',   summary.kegiatan,   summary.total > 0 ? `${((summary.kegiatan   / summary.total) * 100).toFixed(1)}%` : '0%'],
        [],
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      doc.text('Laporan Aktivitas Pojok Statistik BPS Jawa Timur (Monev \'26)', 12, 11.5);
      doc.setFontSize(8);
      ...
      <div className="flex-between" style={{ marginBottom: '1.75rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            Distribusi konten Pojok Statistik Jawa Timur (Periode Monev 2026)
          </p>
        </div>
      ...
      <div className="glass-card" style={{ padding: '1.5rem', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>
            Distribusi Konten per Pojok Statistik (Monev '26)
          </h3>
